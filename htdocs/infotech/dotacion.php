...
$mostra[2]="nombre";
$mostra[3]="cedula";
$otras="AND `personalactivo`.`activo`='$pactivon' AND `personalactivo`.`sucursal` LIKE '$_SESSION[sucur]'";
$cadena=selection("personalactivo","cedula","%",$mostra,$_SESSION[cedulamod],3,"apellidos",$otras);

$resultdartospac=@mysql_query("select cedula, nombre, apellidos, codigo, activo from personalactivo where cedula='$_SESSION[cedulamod]'");

if(@mysql_result($resultdartospac, 0, activo)=="1"){
$estadopac="personalactivo3.php";
}else{
$estadopac="personalretirado3.php";
}

$cadenapaciente="<a href='$estadopac?ejecut=Buscar&amp;criterio=".@mysql_result($resultdartospac, 0, "cedula")."&amp;campobusqueda=Cedula&amp;opt=2' target='_blank'>".@mysql_result($resultdartospac, 0, "cedula")." ".@mysql_result($resultdartospac, 0, "nombre")." ".@mysql_result($resultdartospac, 0, "apellidos")." ".@mysql_result($resultdartospac, 0, "codigo")."</a>";
...
//agregar matriz de datos de grid de resumen
        ...
        $JsonPersAct=devolverSentenciaJson("SELECT apellidos, nombre, cedula FROM personalactivo WHERE personalactivo.activo=1 AND sucursal='$_SESSION[sucur]' ORDER BY apellidos ASC");
        $JsonPersRet=devolverSentenciaJson("SELECT apellidos, nombre, cedula FROM personalactivo WHERE personalactivo.activo=0 AND sucursal='$_SESSION[sucur]' ORDER BY apellidos ASC");
...
?>      
    ...
    function cargarPersonal(tipo){
        ...
        op.value = arrperacts.datos[i].cedula;
        op.text = arrperacts.datos[i].apellidos + " "+ arrperacts.datos[i].nombre+ " "+arrperacts.datos[i].cedula;
        ...
        var op = document.createElement("OPTION");
        op.value = arrperret.datos[i].cedula;
        op.text = arrperret.datos[i].apellidos + " "+ arrperret.datos[i].nombre+" "+arrperret.datos[i].cedula;
        Ext.get('selectpacientes').dom.options[i] = op;
        }
        break;
        }

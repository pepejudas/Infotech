...
validar("","","", 17);
	...
	formularioactual();
		...
		$sqcl="SELECT * FROM dotacion INNER JOIN personalactivo ON dotacion.ceduladot=personalactivo.cedula ORDER BY apellidos";
		...
		case "cargar":
			$_SESSION['i']=0;
			$_SESSION[iddota]=$_POST[cedpersona];
...
$sql1r="SELECT * FROM dotacion, personalactivo, clientes WHERE personalactivo.cedula=dotacion.ceduladot AND personalactivo.codigo=clientes.codigo AND personalactivo.sucursal LIKE '$_SESSION[sucur]' AND personalactivo.activo =1 ORDER BY fechaent DESC LIMIT 100";
...
if(@mysql_result($resultar,$inir,ceduladot)!=@mysql_result($resultar,$inir-1,ceduladot) || @mysql_result($resultar,$inir,iddot)!=@mysql_result($resultar,$inir-1,iddot)){
		if($_SESSION[iddota]==@mysql_result($resultar,$inir-1,iddot)){
			$cadena.="<option value=\"". @mysql_result($resultar,$inir-1,iddot)."\" selected>". @mysql_result($resultar,$inir-1,iddot)." ". @mysql_result($resultar,$inir-1,apellidos)." ".@mysql_result($resultar,$inir-1,nombre)."</option>";
		}else{
			$cadena.="<option value=\"". @mysql_result($resultar,$inir-1,iddot)."\"\> ". @mysql_result($resultar,$inir-1,iddot)." ". @mysql_result($resultar,$inir-1,apellidos)." ".@mysql_result($resultar,$inir-1,nombre)."</option>";
		}
}
$inir++;}
...
<html>
		<head>
		<title>Documentos Dotacion Personal</title>
...
<form method="post" action="<?php echo $PHP_SELF?>">
		...
		<tr>
	  		...
	  		<select name="orden" style="WIDTH: 70%" tabindex="39">
  	 		<option value="ceduladot" <?php  if ($_SESSION['ord']=="ceduladot") {echo ('selected=""');}?>>Cedula</option>
		...
		<tr>
			...
			<?php  if ($_SESSION['ord']=="") {echo ($_SESSION['ord']=="ceduladot");}?>
  	 		<option value="1" <?php  if ($_SESSION['lis']==1) {echo ('selected=""');}?>>Listado personal sin paz y salvo almacen</option>
	 		</select>Listado
	  		</td>
		</tr>
  ...
  echo $nj;?>" target="_blank"><img height="20" alt="" src="imagenes/as01.gif" border=""></a>
	 	...
	 	<?php 
		$parametros[tabla]="dotacion, personalactivo";
		$parametros[campos]="personalactivo.cedula, nombre, apellidos, id, 	iddot, cantidad, compsal, comodot, idprod, fechaent, fechadev, usuaent, usuarec, pazysalvo";
		$parametros[condiciones]="dotacion.ceduladot = personalactivo.cedula AND personalactivo.activo=1 AND personalactivo.sucursal LIKE '$_SESSION[sucur]'";

...
$sql1="SELECT * FROM `personalactivo`, `clientes` WHERE `personalactivo`.`codigo`=`clientes`.`codigo` AND (personalactivo.credsuperintendencia='inserte' or personalactivo.credsuperintendencia='' or personalactivo.credsuperintendencia='NULL' AND personalactivo.oficiotramitecred IS NULL) AND personalactivo.sucursal LIKE '$_SESSION[sucur]' AND personalactivo.activo = 1  AND `personalactivo`.`codigo` LIKE '$_SESSION[clielis]' AND `clientes`.`duenopuesto` LIKE '$_SESSION[sociolis]' ORDER BY $_SESSION[ord]";
//echo $sql1;
...
$texto="* Personal sin credencial SVySP registrada en el sistema";
...
while($vector=mysql_fetch_array($result)){
	...
	$pasado=mysql_result($result,$reg,vencecredsuperintendencia);

...
$cedula=@mysql_result($result,$_SESSION['i'],"cedula");
...
if($_FILES["rutafoto"]['type']!=""){	
    		...
    		$prefijo = substr(md5(uniqid(rand())),0,6);
    	    	...
    	    	$destino =  "docsguardas/".$narch;
                //die($destino);
        		copy($_FILES['rutafoto']['tmp_name'],$destino);
        		$sql="INSERT INTO docspersonal (nombre, ceduladoc, titulo, observaciones) VALUES ('$narch', '$cedula', '$_POST[titulo]', '$_POST[observaciones]')";
        		$sql2="INSERT INTO registrodemodificaciones (fecha, cambio, hechopor, cedulamodificada, tablamod, sucursal) VALUES (NOW(), 'Ingreso de documento', '$_SESSION[persona]','$_SESSION[clientemod]','1', $_SESSION[sucur])";
        		@mysql_query($sql);
        		@mysql_query($sql2);
		    }    		
}
...
function consultardocs($cedula1){
	$sql="SELECT * FROM docspersonal WHERE docspersonal.ceduladoc = '$cedula1' ORDER BY titulo";
	$result2=@mysql_query($sql);

	$docs='<div style="margin-left:-70px"><form action="eliminardoc.php" method="post">';
	while($row = @mysql_fetch_array($result2)){
	$contad++;
	$docs.= '<table class="control2"><tr><td><b>'.$row["titulo"].' </td><td><input name="elimina'.$row["id"].'" type="checkbox"/></td></tr><tr><td rowspan="2"><a href="docsguardas/'.$row["nombre"].'" target="_blank">'.$row[nombre].'</a></td><td valign="top">'.$row["observaciones"].'</td></tr><tr><td valign="top"></td></tr></table><hr style="margin-left:0px;margin-right:30px"/><br>';
	$casilla=$row["id"];
...
<link rel="stylesheet" href="botones.css" type="text/css">
	...
	<title>Documentos de <?php echo $persona;?></title>
...
<br>
<div>
<form action="subirdoc.php" method="post" enctype="multipart/form-data">
...
<div name="listado" style="margin-left:-70px;"><h1>Listado de Archivos de:</h1><h3><?php echo "$persona</h3><h3>CC: $cedula";?></h3></div>
<br>
<?php
echo $docs2;
/*

...
formularioactual();
 	...
 	if($_POST[ejecut]=="Buscar" || $_GET[ejecut]=="Buscar"){
		...
		$datos[opcion]=$_GET[opt];
                ...
                $datos[otraconsulta]="AND departamentos.activo = 1";
		$datos[claveprinc]="id";
		...
		$_POST['sucursal']=$_SESSION[sucur];
                $mens.=armarEjecutarSentencia("departamentos", $_POST, "insert", $_SESSION);
                
                $z=$_POST['codigo'];
                $notifica=new Notificacion();
                $notifica->crearNotificaciones($_SESSION[idusuario], "todos", "Departamento Nuevo", "<a href='departamentos.php?ejecut=Buscar&criterio=$z&campobusqueda=codigo&opt=2' target='blank'>$z</a> ".$z."");
		...
		$senten="UPDATE `departamentos` SET `activo` = '2' WHERE `departamentos`.`id` = '$_SESSION[clientemod]' LIMIT 1";
...
<body<?php if($mens!=""){echo " onload=\"mensajeini('".addslashes($mens)."');return false;\"";}?>>
		...
		<form method="post" action="departamentos.php" name="departamentos" onsubmit="return validadepartamento(boolvalidar);">

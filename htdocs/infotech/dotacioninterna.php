...
$mostra[1]="codigo";
$mostra[2]="departamento";
$mostra[3]="responsable";

$otras="AND `departamentos`.`activo`='$pactivon' AND `departamentos`.`sucursal` LIKE '$_SESSION[sucur]'";
$cadena=selection("departamentos","codigo","%",$mostra,$_SESSION[cedulamod], 3, "codigo", $otras);

$resultdartospac=@mysql_query("select id, codigo, departamento, responsable, activo from departamentos where codigo='$_SESSION[cedulamod]'");

if(@mysql_result($resultdartospac, 0, activo)=="1"){
$estadopac="departamentos.php";
}else{
$estadopac="departinactivos.php";
}

$cadenapaciente="<a href='$estadopac?ejecut=Buscar&amp;criterio=".@mysql_result($resultdartospac, 0, "id")."&amp;campobusqueda=id&amp;opt=2' target='_blank'>".@mysql_result($resultdartospac, 0, "codigo")." ".@mysql_result($resultdartospac, 0, "departamento")."</a>";
...
//agregar matriz de datos de grid de resumen
        ...
        $JsonPersAct=devolverSentenciaJson("SELECT id, departamento, codigo, responsable FROM departamentos WHERE departamentos.activo=1 AND sucursal='$_SESSION[sucur]' ORDER BY codigo ASC");
        $JsonPersRet=devolverSentenciaJson("SELECT id, departamento, codigo, responsable FROM departamentos WHERE departamentos.activo=0 AND sucursal='$_SESSION[sucur]' ORDER BY codigo ASC");
...
?>
    ...
    function cargarPersonal(tipo){
        ...
        op.text = arrperacts.datos[i].codigo + " "+ arrperacts.datos[i].departamento+ " "+arrperacts.datos[i].responsable;
        ...
        op.text = arrperret.datos[i].codigo + " "+ arrperret.datos[i].departamento+ " "+arrperacts.datos[i].responsable;
        Ext.get('selectpacientes').dom.options[i] = op;
        }
        break;
        }

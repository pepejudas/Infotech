...
$fecha=getdate(time());
$sql1="SELECT * FROM novedades, personalactivo WHERE novedades.cedula LIKE personalactivo.cedula AND personalactivo.sucursal LIKE '$_SESSION[sucur]' ORDER BY novedades.$_SESSION[ord]";
...
$com=$fecha[year]*365.25+$fecha[mon]*30.416+.5;
...
if($num > $com){
	...
	$novedad=decod(@mysql_result($result,$reg,novedad));
	...
	$nombre=decod(@mysql_result($resultaq,0,nombre));
	$apellido=decod(@mysql_result($resultaq,0,apellidos));
	$novedad=decod(@mysql_result($result,$reg,novedad));
	$codbusca=decod(@mysql_result($result,$reg,codcliente));
	$v=@mysql_result($result,$reg,fechanov);
	}

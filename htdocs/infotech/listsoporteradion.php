...
$texto="SOPORTE DIARIO DE RADIOPERACION NOCHE $fecha[mday]-$fecha[mon]-$fecha[year]";
...
$sql="SELECT * FROM `personalactivo`,`controlturnos`, `clientes` WHERE `reg$fecha[mday]` LIKE '$_SESSION[persona]' AND `n$fecha[mday]` NOT LIKE '' AND `mescontrol` LIKE '$mesc' AND `controlturnos`.`cedulacontrol`=`personalactivo`.`cedula` AND `personalactivo`.`codigo`=`clientes`.`codigo` ORDER BY `clientes`.`codigo`, `apellidos`";
...
if($ini!=0){
$y+=5;	
$reporteesperado=@mysql_result($result,$ini,"personaln");
...
$texto2=@mysql_result($result,$ini,"n$fecha[mday]");
$pdf->SetFont('Arial','',10);   

if($texto2==1 && $texto0!="" && $texto0!=0){$texto3="Turno Nocturno";}else{$texto3="";}
...
if($ini==$lim){
$y+=7;	
$sql="SELECT SUM(personaln) AS total from clientes WHERE clientes.activo=1 AND clientes.sucursal=$_SESSION[sucur]";
$result=@mysql_query($sql);

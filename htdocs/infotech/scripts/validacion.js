...
function isFecha(dateStr) {
	...
	if (matchArray == null) {
		Sexy.alert("Por favor Diligencia la Fecha Correctamente");
		return false;
	}
...
function validagregaserv(boolvalidar){
	...
	if(boolvalidar){	
		...
		"El campo debe ser numerico") == false) {
			if (checkField(document.agregaserv.personalnocturno, isInteger, false,
                        "El campo debe ser numerico") == false){
                        return false;
                        }
		}
...
function validadepartamento(boolvalidar){
	if(boolvalidar){
		...
		if (checkField(document.departamentos.nombredepto, isAlphabetic, false,"") == false) {
			return false;
		}
		if (checkField(document.departamentos.responsable, isAlphabetic, false,"") == false) {
			return false;
		}
	}
}
function validaradiop(boolvalidar){
	...
	if(boolvalidar){
		...
		Sexy.confirm('Esta reportando más personas de las asignadas al cliente en este turno, desea continuar?',
                {onComplete: function(valor){
                if(valor){
                document.getElementById("ejecut").value="ingresar";
                document.getElementById("radioperacion").submit();
                }
                
                }});
                return false;
		}else if(numreportes<document.getElementById(turno).value){
		Sexy.confirm('Esta reportando menos personas de las asignadas al cliente en este turno, desea continuar?',
                {onComplete: function(valor){
                if(valor){
                document.getElementById("ejecut").value="ingresar";
                document.getElementById("radioperacion").submit();
		...
		Sexy.alert("No ha seleccionado ninguna persona para reporte");	
		return false;
		}
...
function validaradios(boolvalidar){
	if(boolvalidar){
		...
		if (checkField(document.radios.modelo, isAlphabetic, false,"") == false) {
			return false;
		}
            }catch(er){
                alert(er);
...
function validausuarios(boolvalidar){
	try{
		...
		if(document.usuarios.contrasena.value.length < 7){
			Sexy.alert("La contraseña debe ser al menos de 7 caracteres");
			return false;
		}
...
function validaprogramacion(){
	...
	if(document.getElementById("cliente-"+matrizcodigos[i]).checked==true){
		...
		Sexy.alert("Debe Ingresar el numero de personas Diurnas y Nocurnas del Cliente "+matrizcodigos[i]+" para realizar programaci&oacute;n.");	
		return false
		}
...
function valiparprog(){
	...
	Sexy.alert("debe seleccionar al menos un dia de la semana");
	return false;
	}
